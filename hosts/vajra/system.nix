...
}:
{
  ...
  networking = {
    ...
    networkmanager = {
      enable = true;
      # Poll connectivity every 30s so NetworkManager detects a silently
      # dropped link and attempts to re-DHCP instead of waiting for a reboot.
      settings = {
        "connectivity" = {
          "interval" = 30;
          "response" = 5;
  ...
  # r8169 (Realtek RTL8168) is notorious for PCIe ASPM link-drop bugs:
  # once the PCIe link enters L1, the NIC won't re-establish carrier after
  # a link drop, forcing a full reboot to recover. Disabling ASPM is the
  # canonical upstream fix — the r8169 driver exposes no module params.
